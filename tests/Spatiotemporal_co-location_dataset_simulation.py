...
r_grandchild = 2  # 孙子类点空间半径（同现关系半径）
t_grandchild = 2  # 孙子类点时间半径（同现关系半径）
...
def generate_parent_points(n):
    ...
    for _ in range(n):
        ...
        t = np.random.uniform(*ST_T)
        parents.append((x, y, t))
    return parents


def generate_child_points(parent_points, mean_n_child, r, t):
    """
    Step 2: 为每个父类点生成子类点
    :param parent_points:
    ...
    for parent in parent_points:
        ...
        for _ in range(n_child):
            ...
            # 时间位置：在时间区间内均匀分布
            dt = np.random.uniform(-t_child, t_child)
            t = parent[2] + dt

            # 边界裁剪
            x = np.clip(x, ST_X[0], ST_X[1])
            y = np.clip(y, ST_Y[0], ST_Y[1])
            t = np.clip(t, ST_T[0], ST_T[1])
            child_points.append((x, y, t))

    return child_points


def generate_grandchild_points_with_labels(child_points, n_grandchild, r_grandchild, t_grandchild):
    """
    ...
    for child_idx, child in enumerate(child_points):
        for type_idx in range(n_grandchild):
            ...
            dt = np.random.uniform(-t_grandchild, t_grandchild)
            t = child[2] + dt
            ...
            y = np.clip(y, ST_Y[0], ST_Y[1])
            t = np.clip(t, ST_T[0], ST_T[1])
            grandchild_dict[f'type_{type_idx}'].append((x, y, t))
...
def generate_independent_element(n_points: int, element_name: str, cluster_params: Optional[Dict] = None):
    ...
    if cluster_params is None:
        ...
        child_points = generate_child_points(parent_points, mean_n_child, r_child, t_child)
        ...
        if len(child_points) < n_points:
            ...
            t_extra = np.random.uniform(ST_T[0], ST_T[1], n_extra)
            extra_points = np.column_stack([x_extra, y_extra, t_extra])
...
def generate_ST_cooccurrence_pattern_with_labels(n_parent: int = 10, mean_n_child: float = 10, r_child: float = 15,
                                                 t_child: float = 15,
                                                 n_grandchild: int = 3, r_grandchild: float = 5,
                                                 t_grandchild: float = 5,
    ...
    pattern_labels = {element_names.get(k, k): v for k, v in pattern_labels.items()}
    ...
    element_D_points, element_D_labels, = generate_independent_element(n_points=160, element_name='D',
                                                                       cluster_params=d_params)

    # 要素 E：完全随机分布
    element_E_points, element_E_labels = generate_independent_element(n_points=160, element_name='E',
                                                                      cluster_params=None)
    # 将D和E添加到字典中
    ...
    for elem_type, points in point_dict.items():
        ...
        #     df['element_type'] = type_mapping[elem_type]
        # else:
        #     df['element_type'] = elem_type
        # print(df['element_type'])
        # df['element_type'] = elem_type
    ...
    print(result_df['element_type'].unique())
...
def plot_3d(data):
    ...
    ax.set_zlabel('T', fontsize=24, labelpad=8)

    # ax.legend(loc='best',)
    saved_folder = r'E:\OneDrive\成果\01-论文\21-运动模式挖掘工具箱\figures\datasets'
    plt.savefig(rf'{saved_folder}\ST_co-location.svg', bbox_inches='tight')
    plt.savefig(rf'{saved_folder}\ST_co-location.jpg', bbox_inches='tight', dpi=300)
    plt.savefig(rf'{saved_folder}\ST_co-location.tif', bbox_inches='tight', dpi=300)
    plt.show()


def simulate_spatiotemporal_co_locations():
    print("生成具有预设时空同现模式的数据...")

    cooccurrence_data, cooccurrence_df = generate_ST_cooccurrence_pattern_with_labels(n_parent=n_parent,
                                                                                      mean_n_child=mean_n_child,
                                                                                      r_child=r_child, t_child=t_child,
                                                                                      n_grandchild=n_grandchild,
                                                                                      r_grandchild=r_grandchild,
                                                                                      t_grandchild=t_grandchild,
                                                                                      missing_rates=missing_rates,
                                                                                      noise_rate=noise_rate)
    cooccurrence_df.to_csv('spatiotemporal_co_locations.csv', index=False)

    return cooccurrence_data


if __name__ == "__main__":
    data = simulate_spatiotemporal_co_locations()
    plot_3d(data)
